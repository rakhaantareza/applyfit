...
test("session bootstrap is owned by the persistent root provider", async () => {
  ...
  assert.match(routeSource, /if \(loading \|\| !user\) \{\s*return null;\s*\}/);
  assert.doesNotMatch(
    routeSource,
    /Menyiapkan ruang kerjamu|session-route-check|session-logo-flip|WorkspaceContentLoadingState|AppSidebar/,
  );
...
test("page data loading never renders page-level loaders", async () => {
  const loadingSources = await Promise.all([
    ...
    "../app/portfolio-pengalaman/EvidenceLibraryWorkspace.tsx",
    "../app/lowongan/JobsWorkspace.tsx",
    "../app/components/FitScoreWorkspace.tsx",
    "../app/lowongan/[id]/PersistedJobDetail.tsx",
    "../app/lowongan/[id]/persyaratan/RequirementReviewWorkspace.tsx",
    "../app/lowongan/[id]/cocokkan-profil/EvidenceMappingPageWorkspace.tsx",
