...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE JSONB_TYPEOF(item) <> 'object'
      ...
      OR item->>'type' NOT IN ('skill', 'tool', 'education', 'experience')
      OR item->>'priority' NOT IN ('required', 'preferred')
      OR item ? 'status'
  ) THEN
  ...
  LOOP
    ...
    IF item_id IS NULL THEN
      ...
      VALUES (
        target_job_id,
        BTRIM(item->>'name'),
        (item->>'type')::public.requirement_type,
        (item->>'priority')::public.requirement_priority
      );
    ELSE
      UPDATE public.job_requirements AS requirement
      SET
        name = BTRIM(item->>'name'),
        type = (item->>'type')::public.requirement_type,
        priority = (item->>'priority')::public.requirement_priority
