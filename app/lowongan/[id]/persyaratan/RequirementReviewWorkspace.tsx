...
export function RequirementReviewWorkspace({ jobId }: { jobId: string }) {
        ...
        if (!requirementResponse.ok || !requirementResult.data?.requirements) {
          throw new Error(
            requirementResult.error?.message ??
              "Requirement belum dapat dimuat.",
          );
        }

        const extractedDraft = readExtractionDraft(jobId);
        const initial = extractedDraft.length
          ? extractedDraft.map((requirement, index) =>
              fromExtraction(requirement, index),
            )
  ...
  if (!job || !requirements)
    return <ReviewLoadState error={error} jobId={jobId} />;
  ...
  return (
    ...
    >
      <div className="page-container requirement-review-page">
        ...
        <section
          className="requirement-review-next-step"
          aria-label={t("Langkah berikutnya")}
        >
          <span aria-hidden="true">
            <Waypoints size={19} strokeWidth={1.8} />
          </span>
          <div>
            <strong>{t("Lanjutkan ke Cocokkan Profil")}</strong>
            <p>
              {t(
          ...
          <ActionLink href={`/lowongan/${job.id}/cocokkan-profil`}>
            {t("Buka Cocokkan Profil")} <CtaArrow />
          </ActionLink>
        </section>
      </div>
    </JobFocusShell>
      ...
      <div className="page-container requirement-review-page">
        {t(error) ? (
          <div className="persisted-job-state error">
            ...
            <InlineBackLink href={`/lowongan/${jobId}`}>
              {t("Kembali ke detail lowongan")}
            </InlineBackLink>
          </div>
        ) : null}
      </div>
...
function fromExtraction(
  requirement: Omit<ApiRequirement, "id">,
  index: number,
): Requirement {
  return {
    id: `extracted-${index}`,
...
function isExtractionRequirement(
  value: unknown,
): value is Omit<ApiRequirement, "id"> {
  if (!value || typeof value !== "object") return false;
  const candidate = value as Record<string, unknown>;
  return (
    typeof candidate.name === "string" &&
    candidate.name.trim() !== "" &&
    ["skill", "tool", "education", "experience"].includes(
