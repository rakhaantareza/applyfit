    ...
    async function load() {
      try {
        ...
        if (!jobResponse.ok || !jobResult.data?.job) {
          throw new Error(
            jobResult.error?.message ?? "Konteks lowongan belum dapat dimuat.",
          );
        }
        if (!skillsResponse.ok || !skillsResult.data?.skills) {
          throw new Error(
            skillsResult.error?.message ?? "Skill profil belum dapat dimuat.",
          );
        }
        ...
        const autoMatchResult =
          await readJson<AutoMatchResponse>(autoMatchResponse);
        const exactMatches = autoMatchResponse.ok
          ? (autoMatchResult.data?.matches ?? [])
          : [];
        ...
        if (!summaryResponse.ok || !summaryResult.data) {
          throw new Error(
            summaryResult.error?.message ??
              "Cocokkan Profil belum dapat dimuat.",
          );
        }
        ...
        const skillEvidences = await Promise.all(
          apiSkills.map(async (skill) => {
            const response = await fetch(
              `/api/evidences?skillId=${encodeURIComponent(skill.id)}`,
              { cache: "no-store" },
            );
        ...
        setMapping({
          skills: apiSkills.map((skill, index) => ({
            ...
            evidence:
              skillEvidences[index]?.map(({ id, title }) => ({ id, title })) ??
              [],
          })),
          requirements: summaryResult.data.requirements.map((requirement) => {
            ...
            return {
              ...
              priority:
                requirement.priority === "required" ? "Wajib" : "Preferensi",
          ...
          informationalRequirements:
            summaryResult.data.informationalRequirements,
        });
      } catch (requestError) {
        if (active)
          setError(
  ...
  return (
    ...
    >
      <div className="page-container evidence-mapping-page">
        ...
        <section
          className="mapping-job-context"
          aria-labelledby="mapping-job-title"
        >
          <span className="mapping-job-mark" aria-hidden="true">
            {getInitials(job.company)}
          </span>
          <div className="mapping-job-copy">
            <p className="eyebrow">{t("Lowongan")}</p>
            <h2 id="mapping-job-title">{job.title}</h2>
            <strong>{job.company}</strong>
          </div>
          <div className="mapping-job-meta" aria-label={t("Konteks lowongan")}>
            <span>
              <BriefcaseBusiness
                aria-hidden="true"
                size={14}
        ...
        {mapping.informationalRequirements.length > 0 ? (
          <details className="mapping-informational">
            <summary>
              ...
              <span>
                <strong>
                  {mapping.informationalRequirements.length}{" "}
                  {t("persyaratan sebagai konteks")}
                </strong>
                <small>
                  {t("Pengalaman dan pendidikan tidak masuk Fit Score.")}
                </small>
              </span>
        ...
        <div className="mapping-evidence-note">
          ...
          <p>
            {t("Bukti dikelola di")}{" "}
            <Link href="/portfolio-pengalaman">
              {t("Portfolio & Pengalaman")}
            </Link>{" "}
            {t("dan dapat digunakan kembali untuk lowongan lain.")}
          </p>
        </div>
      </div>
    </JobFocusShell>
      ...
      <div className="page-container evidence-mapping-page">
        {t(error) ? (
          <div className="persisted-job-state error">
            ...
            <InlineBackLink href={`/lowongan/${jobId}/persyaratan`}>
              {t("Kembali ke review requirement")}
            </InlineBackLink>
          </div>
        ) : null}
      </div>
