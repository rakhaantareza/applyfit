...
export function EvidenceLibraryWorkspace() {
  ...
  useEffect(() => {
    ...
    async function loadLibrary() {
      try {
        const response = await fetch("/api/workspace?scope=portfolio", {
          cache: "no-store",
        });
        ...
        const skillsById = new Map(
          profileSkills.map((skill) => [skill.id, skill]),
        );

        if (!active) return;
        setSkills(profileSkills);
        setEvidences(
          apiEvidences.map((evidence) => ({
            id: evidence.id,
            ...
            source: evidence.url,
            skills:
              evidence.skillIds?.flatMap((skillId) => {
  ...
  return (
    <EvidenceLibrary initialEvidences={evidences} availableSkills={skills} />
  );
}
